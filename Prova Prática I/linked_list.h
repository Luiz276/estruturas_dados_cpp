...
void structures::LinkedList<T>::invert() {
    ...
    head = prev;
    /*
    Node* aux1;  // nodo da frente
    Node* aux2;  // nodo de trás
    Node* aux3;  // nodo anterior ao de trás
    Node* auxNext;
    for (int i = 0; i < static_cast<int>(size())/2; i++) {
        aux1 = before_index(i+1);
        aux2 = before_index(static_cast<int>(size())-i);
        aux3 = before_index(static_cast<int>(size())-i-1);
...
void structures::LinkedList<T>::append(structures::LinkedList<T> &list_add) {
    // ...
    Node* aux;
    for (int i = 1; i <= static_cast<int>(list_add.size()); i++) {
        aux = list_add.before_index(i);
        push_back(aux->data());
    }
    // ...
}
...
structures::LinkedList< structures::LinkedList<T> * >
    ...
    // ...
    LinkedList<T> * par = new LinkedList<T>();
    LinkedList<T> * impar = new LinkedList<T>();
    Node* aux;
    for (int i = 1; i <= static_cast<int>(size()); i++) {
        aux = before_index(i);
        if (i % 2 != 0) {
            par->push_back(aux->data());
        } else {
            impar->push_back(aux->data());
        }
    }
    list_halve.push_front(par);
    list_halve.push_back(impar);
    // ...
    return list_halve;
}
